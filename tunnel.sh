...
#================================================
# VPN Auto Script v3.0 - FIXED COMPLETE
...
draw_box_top() {
    ...
    case "$theme" in
        ...
        "colorful")
            ...
            ;;
        "classic")
            echo -e "${WHITE}+$(printf -- '-%.0s' $(seq 1 $BOX_WIDTH))+${NC}"
            ;;
    esac
...
print_section() {
    ...
    case "$theme" in
        "modern"|"colorful")
            printf "${border}║${NC} %b%*s${border}║${NC}\n" "$title" "$padding" ""
            ;;
        "minimal")
            printf "${border}│${NC} %b%*s${border}│${NC}\n" "$title" "$padding" ""
            ;;
...
print_info() {
    ...
    case "$theme" in
        "modern"|"colorful")
            printf "${border}║${NC} ${WHITE}%s${NC}${separator}%b%*s${border}║${NC}\n" \
                "$label" "$value" "$padding" ""
            ;;
        "minimal")
            printf "${border}│${NC} ${WHITE}%s${NC}${separator}%b%*s${border}│${NC}\n" \
...
print_menu_option() {
    ...
    local num_len=${#num}
    local text_len=${#text}
    local total=$((5 + num_len + text_len))
    local padding=$((BOX_WIDTH - total - 2))
    
    case "$theme" in
        "modern"|"colorful")
            printf "${border}║${NC}  ${YELLOW}[%s]${NC} %s%*s${border}║${NC}\n" \
                "$num" "$text" "$padding" ""
            ;;
        "minimal")
            printf "${border}│${NC}  ${CYAN}[%s]${NC} %s%*s${border}│${NC}\n" \
                "$num" "$text" "$padding" ""
            ;;
        "classic")
            printf "${border}|${NC}  ${YELLOW}[%s]${NC} %s%*s${border}|${NC}\n" \
...
print_menu_double() {
    ...
    local item1_len=$((5 + ${#num1} + ${#text1}))
    local item2_len=$((5 + ${#num2} + ${#text2}))
    local padding1=$((col_width - item1_len))
    local padding2=$((BOX_WIDTH - col_width - item2_len - 2))
    
    case "$theme" in
        "modern"|"colorful")
            printf "${border}║${NC}  ${YELLOW}[%s]${NC} %s%*s${YELLOW}[%s]${NC} %s%*s${border}║${NC}\n" \
                "$num1" "$text1" "$padding1" "" "$num2" "$text2" "$padding2" ""
            ;;
        "minimal")
            printf "${border}│${NC}  ${CYAN}[%s]${NC} %s%*s${CYAN}[%s]${NC} %s%*s${border}│${NC}\n" \
                "$num1" "$text1" "$padding1" "" "$num2" "$text2" "$padding2" ""
            ;;
        "classic")
            printf "${border}|${NC}  ${YELLOW}[%s]${NC} %s%*s${YELLOW}[%s]${NC} %s%*s${border}|${NC}\n" \
...
print_status() {
    ...
    local total=$((3 + label_len + status_len))
    local padding=$((BOX_WIDTH - total - 2))
    
    local char_border
    case "$theme" in
        "modern"|"colorful") char_border="║" ;;
        "minimal") char_border="│" ;;
        "classic") char_border="|" ;;
    esac
    
    if [[ "$status" == "RUNNING" ]] || [[ "$status" == "ON" ]]; then
        printf "${border}%s${NC} ${GREEN}●${NC} %-20s ${GREEN}%s${NC}%*s${border}%s${NC}\n" \
            "$char_border" "$label" "$status" "$padding" "" "$char_border"
    else
        printf "${border}%s${NC} ${RED}○${NC} %-20s ${RED}%s${NC}%*s${border}%s${NC}\n" \
            "$char_border" "$label" "$status" "$padding" "" "$char_border"
    fi
}
...
select_menu_theme() {
    clear
    echo ""
    draw_box_top "🎨  PILIH TEMA TAMPILAN MENU"
    print_empty
    print_section "${BOLD}${WHITE}TEMA 1 - MODERN BOX (Default)${NC}"
    print_section "  ✓ Double border tebal (╔═╗)"
    print_section "  ✓ Color-coded sections"
    print_section "  ✓ Professional look"
    print_empty
    draw_separator
    print_section "${BOLD}${WHITE}TEMA 2 - MINIMALIST${NC}"
    print_section "  ✓ Single line border (┌─┐)"
    print_section "  ✓ Clean & simple"
    print_empty
    draw_separator
    print_section "${BOLD}${WHITE}TEMA 3 - COLORFUL${NC}"
    print_section "  ✓ Rounded corners (╭─╮)"
    print_section "  ✓ Vibrant colors"
    print_empty
    draw_separator
    print_section "${BOLD}${WHITE}TEMA 4 - CLASSIC${NC}"
    print_section "  ✓ ASCII style (+--+)"
    print_section "  ✓ Universal compatibility"
    print_empty
    draw_box_bottom
    echo ""
    echo -ne "  ${CYAN}➤${NC} Pilihan ${WHITE}[1-4]${NC}: "
    read theme_choice
...
setup_domain() {
    ...
    echo -e "${GREEN}┃${NC}                                                                  ${GREEN}┃${NC}"
    echo -e "${GREEN}┃${NC}  ${CYAN}Contoh:${NC}                                                        ${GREEN}┃${NC}"
    echo -e "${GREEN}┃${NC}    ${WHITE}vpn.example.com${NC}                                             ${GREEN}┃${NC}"
    echo -e "${GREEN}┃${NC}    ${WHITE}server.namadomain.com${NC}                                       ${GREEN}┃${NC}"
    echo -e "${GREEN}┃${NC}                                                                  ${GREEN}┃${NC}"
    ...
    echo -e "${BLUE}┃${NC}                                                                  ${BLUE}┃${NC}"
    echo -e "${BLUE}┃${NC}  ${CYAN}Preview domain yang akan dibuat:${NC}                             ${BLUE}┃${NC}"
    printf "${BLUE}┃${NC}    ${BOLD}${WHITE}%-62s${NC}${BLUE}┃${NC}\n" "$domain_preview"
    echo -e "${BLUE}┃${NC}                                                                  ${BLUE}┃${NC}"
    ...
    case $domain_choice in
        1)
            ...
            echo -e "  • Domain harus pointing ke IP: ${GREEN}${ip_preview}${NC}"
            echo -e "  • Cek dengan: ${CYAN}ping namadomain.com${NC}"
            echo ""
            ...
            print_info "Status" "${YELLOW}Akan diverifikasi saat instalasi${NC}"
            print_empty
            draw_box_bottom
            ;;
...
#================================================
# DASHBOARD - FIXED PERFECT ALIGNMENT
#================================================
...
show_system_info() {
    ...
    # Display
    echo ""
    draw_box_top "VPN SERVER DASHBOARD v3.0"
    print_section "${GREEN}Proffessor Squad${NC} • ${YELLOW}@ridhani16${NC}"
    draw_separator
    
    # Server Info
    print_section "${BOLD}${WHITE}SERVER INFORMATION${NC}"
    print_empty
    ...
    # Accounts Summary
    print_section "${BOLD}${WHITE}ACCOUNTS SUMMARY${NC}"
    print_empty
    ...
    # Service Status
    print_section "${BOLD}${WHITE}SERVICE STATUS${NC}"
    print_empty
...
show_menu() {
    ...
    # Account Management
    print_section "${BOLD}${WHITE}ACCOUNT MANAGEMENT${NC}"
    print_empty
    ...
    # System Tools
    print_section "${BOLD}${WHITE}SYSTEM TOOLS${NC}"
    print_empty
    ...
    # Advanced
    print_section "${BOLD}${WHITE}ADVANCED${NC}"
    print_empty
    ...
    echo ""
    echo -e "${YELLOW}💡 TIP:${NC} Type ${WHITE}help${NC} for guide  │  ${YELLOW}📞${NC} Support: ${WHITE}@ridhani16${NC}"
    echo ""
}

#================================================
# PROTOCOL MENUS - FIXED
#================================================

menu_ssh() {
    while true; do
        clear
        draw_box_top "SSH MENU"
...
menu_trial_generator() {
    clear
    draw_box_top "TRIAL ACCOUNT GENERATOR"
...
show_info_port() {
    clear
    draw_box_top "SERVER PORT INFORMATION"
    print_empty
    ...
    print_empty
    draw_separator
    print_section "${BOLD}${WHITE}PATH CONFIGURATION${NC}"
    print_empty
...
list_all_accounts() {
    ...
    if [[ -f "${vmess_files[0]}" ]]; then
        print_section "${BOLD}${GREEN}VMESS ACCOUNTS${NC}"
        print_empty
    ...
    if [[ -f "${vless_files[0]}" ]]; then
        print_section "${BOLD}${GREEN}VLESS ACCOUNTS${NC}"
        print_empty
    ...
    if [[ -f "${trojan_files[0]}" ]]; then
        print_section "${BOLD}${GREEN}TROJAN ACCOUNTS${NC}"
        print_empty
...
cek_expired() {
    ...
    for f in "$AKUN_DIR"/*.txt; do
        ...
        if [[ $diff -le 3 ]]; then
            found=1
            if [[ $diff -lt 0 ]]; then
                print_section "${RED}EXPIRED${NC}: $uname ($exp_str)"
            else
                print_section "${YELLOW}$diff days${NC}: $uname ($exp_str)"
            fi
        fi
    done
    shopt -u nullglob
...
delete_expired() {
    clear
    draw_box_top "DELETE EXPIRED ACCOUNTS"
    print_empty
...
show_help() {
    ...
    print_empty
    print_section "${BOLD}${WHITE}Account Management:${NC}"
    print_section "  ${CYAN}1-4${NC}   → Protocol account menus"
    print_section "  ${CYAN}5${NC}     → Generate trial accounts (1 hour)"
    ...
    print_section "  ${CYAN}12-13${NC} → VPS optimization & restart"
    print_section "  ${CYAN}14-15${NC} → Service info & speedtest"
    print_section "  ${CYAN}16${NC}    → Update script from GitHub"
    print_section "  ${CYAN}17-18${NC} → Backup & restore system"
    print_empty
    print_section "${BOLD}${WHITE}Advanced:${NC}"
    print_section "  ${CYAN}19${NC}    → Uninstall components"
    print_section "  ${CYAN}20${NC}    → Change menu theme"
    print_section "  ${CYAN}99${NC}    → Advanced settings (12 menus)"
...
SSHEOF
    ...
    draw_separator
    print_section "${BOLD}${WHITE}PORT CONFIGURATION${NC}"
    print_empty
...
DLEOF
    ...
    print_empty
    print_section "${CYAN}NonTLS Link:${NC}"
...
create_trial_account() {
    ...
    if [[ "$protocol" == "ssh" ]]; then
        ...
        clear
        draw_box_top "SSH TRIAL ACCOUNT"
        print_empty
        print_info "Username" "$username"
        print_info "Password" "$password"
        print_info "Domain" "$DOMAIN"
        ...
        clear
        draw_box_top "${protocol^^} TRIAL ACCOUNT"
        print_empty
        print_info "Username" "$username"
        print_info "UUID" "$uuid"
        print_info "Domain" "$DOMAIN"
...
delete_account() {
    local protocol="$1"
    clear
    draw_box_top "DELETE ${protocol^^} ACCOUNT"
    print_empty
    ...
    for f in "${files[@]}"; do
        ...
        print_section "  ${CYAN}•${NC} $n ${YELLOW}($e)${NC}"
    done
...
list_protocol_accounts() {
    ...
    if [[ ${#files[@]} -eq 0 ]]; then
        ...
        for f in "${files[@]}"; do
            ...
            if [[ "$trial" == "1" ]]; then
                print_section "${YELLOW}[TRIAL]${NC} $uname - Exp: $exp"
            else
                print_section "${GREEN}[MEMBER]${NC} $uname - Quota: ${quota}GB - Exp: $exp"
            fi
        done
        print_empty
...
check_user_login() {
    local protocol="$1"
    clear
    draw_box_top "ACTIVE ${protocol^^} LOGINS"
    print_empty
    
    if [[ "$protocol" == "ssh" ]]; then
        print_section "${WHITE}Active SSH Sessions:${NC}"
        print_empty
        ...
        if [[ -z "$active_users" ]]; then
            ...
            while IFS= read -r user; do
                ...
                print_section "${GREEN}●${NC} User: ${WHITE}$user${NC}"
                print_section "  Connections: ${CYAN}$login_count${NC}"
                print_section "  Login Time: ${CYAN}$login_time${NC}"
                print_empty
            done <<< "$active_users"
        fi
    else
        print_section "${WHITE}Active ${protocol^^} Connections:${NC}"
        print_empty
            ...
            if [[ -z "$recent_logs" ]]; then
                ...
                if [[ -z "$active_users" ]]; then
                    ...
                    while IFS= read -r username; do
                        ...
                        print_section "${GREEN}●${NC} User: ${WHITE}$username${NC}"
...
_adv_port_management() {
    ...
    print_empty
    print_section "${BOLD}${WHITE}Current Port Configuration:${NC}"
    print_empty
    print_info "SSH OpenSSH" "22"
    print_info "SSH Dropbear" "222"
    print_info "Nginx HTTP" "80"
...
_adv_auto_backup() {
    clear
    draw_box_top "AUTO BACKUP CONFIGURATION"
    ...
    case $backup_choice in
        ...
        5)
            ...
            if [[ -d /root/backups ]]; then
                local backups=$(ls -1 /root/backups/*.tar.gz 2>/dev/null)
                if [[ -n "$backups" ]]; then
                    while IFS= read -r backup; do
                        local filename=$(basename "$backup")
                        local size=$(du -h "$backup" | awk '{print $1}')
                        print_section "  $filename - $size"
                    done <<< "$backups"
                else
                    print_section "${YELLOW}No backups found${NC}"
                fi
            else
                print_section "${YELLOW}Backup directory not found${NC}"
            fi
            print_empty
            draw_box_bottom
            echo ""
...
_adv_ssh_protection() {
    ...
    case $ssh_choice in
        ...
        4)
            clear
            draw_box_top "FAILED SSH LOGINS"
            print_empty
            draw_box_bottom
            echo ""
...
_adv_custom_scripts() {
    ...
    if [[ -d /root/scripts ]]; then
        local scripts=$(ls -1 /root/scripts/*.sh 2>/dev/null)
        if [[ -n "$scripts" ]]; then
            while IFS= read -r script; do
                print_section "  • $(basename "$script")"
            done <<< "$scripts"
        else
            print_section "${YELLOW}No scripts found${NC}"
        fi
    else
        print_section "${YELLOW}Directory not found${NC}"
        print_section "  Create: ${CYAN}mkdir -p /root/scripts${NC}"
    fi
    print_empty
    draw_box_bottom
    echo ""
...
update_menu() {
    ...
    echo ""
    echo -e "${CYAN}[1/4]${NC} Backup..."
    cp "$SCRIPT_PATH" "$BACKUP_PATH" 2>/dev/null
    [[ $? -eq 0 ]] && echo -e "      ${GREEN}✓${NC}" || { echo -e "      ${RED}✗${NC}"; return; }
    
    echo -e "${CYAN}[2/4]${NC} Download..."
    local tmp="/tmp/tunnel_new.sh"
    curl -sL "$SCRIPT_URL" -o "$tmp"
    [[ -s "$tmp" ]] && echo -e "      ${GREEN}✓${NC}" || { echo -e "      ${RED}✗${NC}"; return; }
    
    echo -e "${CYAN}[3/4]${NC} Validate..."
    bash -n "$tmp" 2>/dev/null
    [[ $? -eq 0 ]] && echo -e "      ${GREEN}✓${NC}" || { echo -e "      ${RED}✗${NC}"; rm -f "$tmp"; return; }
    
    echo -e "${CYAN}[4/4]${NC} Apply..."
    mv "$tmp" "$SCRIPT_PATH"
    chmod +x "$SCRIPT_PATH"
    echo -e "      ${GREEN}✓${NC}"
...
change_domain() {
    ...
    setup_domain
    echo ""
    echo -e "${YELLOW}⚠ Please run 'Fix SSL/Cert' menu!${NC}"
    sleep 3
}
...
setup_telegram_bot() {
    ...
    print_empty
    print_section "${WHITE}How to get Bot Token:${NC}"
    print_section "  1. Open Telegram, search ${CYAN}@BotFather${NC}"
    print_section "  2. Type /newbot and follow instructions"
    print_section "  3. Copy the TOKEN provided"
    print_empty
    print_section "${WHITE}How to get Chat ID:${NC}"
    print_section "  1. Search ${CYAN}@userinfobot${NC} in Telegram"
    print_section "  2. Type /start to see your ID"
    print_empty
    draw_box_bottom
    echo ""
...
uninstall_haproxy() {
    clear
    draw_box_top "UNINSTALL HAPROXY"
    print_empty
    draw_box_bottom
    echo ""
    read -p " Continue? [y/N]: " c
...
uninstall_dropbear() {
    clear
    draw_box_top "UNINSTALL DROPBEAR"
    print_empty
    draw_box_bottom
    echo ""
    read -p " Continue? [y/N]: " c
...
uninstall_udp() {
    clear
    draw_box_top "UNINSTALL UDP CUSTOM"
    print_empty
    draw_box_bottom
    echo ""
    read -p " Continue? [y/N]: " c
...
uninstall_bot() {
    clear
    draw_box_top "UNINSTALL TELEGRAM BOT"
    print_empty
    draw_box_bottom
    echo ""
    read -p " Continue? [y/N]: " c
...
uninstall_keepalive() {
    clear
    draw_box_top "UNINSTALL KEEPALIVE"
    print_empty
    draw_box_bottom
    echo ""
    read -p " Continue? [y/N]: " c
...
smart_install() {
    ...
    local base_pkgs=(
        curl wget unzip uuid-runtime net-tools openssl jq qrencode 
    ...
    # Netdata (optional) - SAFE INSTALL
    if is_installed "netdata"; then
        _skip "Netdata"
    else
        echo -e "${CYAN}Installing Netdata (optional, may take time)...${NC}"
        (
            timeout 120 bash <(curl -Ss https://my-netdata.io/kickstart.sh) --dont-wait --non-interactive >> "$LOG" 2>&1
        ) && _ok "Netdata" || _skip "Netdata (optional, skipped)"
    fi
    ...
    if is_installed "netdata"; then
        ...
        echo -e "${CYAN}Installing Netdata (optional)...${NC}"
