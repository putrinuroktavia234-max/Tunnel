...
DURATION_MONTHLY="30"; STUNNEL_SSH_PORT="444"; STUNNEL_DROP_PORT="445"
...
change_domain() {
    ...
    local _cur_dom="${DOMAIN:-Not Set}"; local _cur_dom_len="${#_cur_dom}"
    _box_row "${WHITE}Current :${NC} ${_cur_dom}" $(( 10 + _cur_dom_len ))
    _box_bot; echo ""
...
show_system_info() {
    ...
    local svc_total=9 svc_on=0
    local svcs=("xray" "nginx" "sshd" "dropbear" "haproxy" "stunnel4" "udp-custom" "vpn-bot" "vpn-keepalive")
    ...
    local st_stun=$(check_status "stunnel4")
    ...
    _box_mid
    local _dom_disp="${DOMAIN:-Not Set}"; local _dom_len="${#_dom_disp}"
    _box_row "${WHITE}Domain  :${NC} ${CYAN}${_dom_disp}${NC}" $(( 10 + _dom_len ))
    ...
    _svc2 "OPENSSH"  "$st_ssh"    "STUNNEL4" "$st_stun"
    _svc2 "UDP CUST" "$st_udp"    "TELEGRAM" "$st_tg"
    _svc2 "KEEPALIV" "$st_ka"     "VPN-BOT"  "$st_tg"
    _box_bot
}
...
show_menu() {
    ...
    _box_2col "${CYAN}[21]${NC} Stunnel4 Setup"  ""                               $(( 4+14 )) 0
    _box_mid
...
show_info_port() {
    ...
    _box_row "${WHITE}Stunnel SSH  :${NC} ${STUNNEL_SSH_PORT} (SSL over SSH)" $(( 14 + ${#STUNNEL_SSH_PORT} + 15 ))
    _box_row "${WHITE}Stunnel Drop :${NC} ${STUNNEL_DROP_PORT} (SSL over Dropbear)" $(( 14 + ${#STUNNEL_DROP_PORT} + 20 ))
...
Username      : ${usr}
Password      : ${pass}
Max Login     : ${maxlogin} IP
Port SSH      : 22, 222
Port SSL/STN  : ${STUNNEL_SSH_PORT}, ${STUNNEL_DROP_PORT}
Port TLS      : 443
Payload       : GET / HTTP/1.1[crlf]Host: ${DOMAIN}[crlf][crlf]
◇━━━━━━━━━━━━━━━━━━━━◇
Aktif Selama  : ${days} Hari
Dibuat Pada   : $(date +"%d %b, %Y")
Berakhir Pada : $(date -d "+${days} days" +"%d %b, %Y")
◇━━━━━━━━━━━━━━━━━━━━◇"
    send_telegram_admin "$ssh_tg_msg"
    read -p " Press any key..."
}
...
create_vmess() {
    clear; _box_top; _box_center "CREATE VMESS ACCOUNT"; _box_mid; echo ""
    _box_center "VMess / WebSocket + TLS" "$YELLOW"; _box_bot; echo ""
    read -p " Username: " usr; [[ -z "$usr" ]] && { echo -e "${RED}Empty!${NC}"; sleep 2; return; }
    read -p " Duration (days) [30]: " days; days=${days:-30}
    read -p " Quota limit GB [unlimited]: " quota; quota=${quota:-10000}
    read -p " Max IP limit [1]: " maxip; maxip=${maxip:-1}
    local uuid=$(_gen_uuid)
    local exp=$(date -d "+${days} days" +"%Y-%m-%d")
    local created=$(date +"%Y-%m-%d")
    local created_fmt=$(date +"%d %b, %Y")
    local exp_fmt=$(date -d "+${days} days" +"%d %b, %Y")

    _add_xray_user "vmess" "$uuid" "$usr" || return
    systemctl restart xray 2>/dev/null
    mkdir -p "$AKUN_DIR/vmess"
    echo "{\"username\":\"${usr}\",\"uuid\":\"${uuid}\",\"created\":\"${created}\",\"expired\":\"${exp}\",\"duration\":\"${days}\",\"quota\":\"${quota}\",\"maxip\":\"${maxip}\"}" > "$AKUN_DIR/vmess/${usr}.json"

    local host="$DOMAIN" bug="bug.com"
    # WS TLS link (port 443 via haproxy)
    local obj_tls="{\"v\":\"2\",\"ps\":\"${usr}\",\"add\":\"${bug}\",\"port\":\"443\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"path\":\"/vmess\",\"type\":\"none\",\"host\":\"${host}\",\"tls\":\"tls\"}"
    local link_tls="vmess://$(echo -n "$obj_tls" | base64 -w0)"
    # WS nonTLS link (port 80)
    local obj_ntls="{\"v\":\"2\",\"ps\":\"${usr}\",\"add\":\"${bug}\",\"port\":\"80\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"path\":\"/vmess\",\"type\":\"none\",\"host\":\"${host}\",\"tls\":\"none\"}"
    local link_ntls="vmess://$(echo -n "$obj_ntls" | base64 -w0)"
    # gRPC link
    local obj_grpc="{\"v\":\"2\",\"ps\":\"${usr}\",\"add\":\"${host}\",\"port\":\"443\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"grpc\",\"path\":\"vmess-grpc\",\"type\":\"none\",\"host\":\"${bug}\",\"tls\":\"tls\"}"
    local link_grpc="vmess://$(echo -n "$obj_grpc" | base64 -w0)"
    ...
    echo -e " Limit User (GB): ${quota}"
    echo -e " Limit User (IP): ${maxip}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    ...
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Link TLS         : ${link_tls}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Link none TLS    : ${link_ntls}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Link GRPC        : ${link_grpc}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Format OpenClash : https://${host}:81/vmess-${usr}.txt"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Aktif Selama     : ${days} Hari"
    echo -e " Dibuat Pada      : ${created_fmt}"
    echo -e " Berakhir Pada    : ${exp_fmt}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
...
◇━━━━━━━━━━━━━━━━━━━━◇
Aktif Selama  : ${days} Hari
Dibuat Pada   : ${created_fmt}
Berakhir Pada : ${exp_fmt}
◇━━━━━━━━━━━━━━━━━━━━◇"
    send_telegram_admin "$tg_msg"
    echo ""; read -p " Press any key..."
}
...
create_vless() {
    ...
    echo "{\"username\":\"${usr}\",\"uuid\":\"${uuid}\",\"created\":\"$(date +%Y-%m-%d)\",\"expired\":\"${exp}\",\"duration\":\"${days}\",\"quota\":\"${quota}\",\"maxip\":\"${maxip}\"}" > "$AKUN_DIR/vless/${usr}.json"

    local host="$DOMAIN"
    local link_tls="vless://${uuid}@${host}:443?security=tls&sni=${host}&flow=xtls-rprx-vision&type=tcp#${usr}"
    local link_ws="vless://${uuid}@${host}:80?security=none&type=ws&path=/vless&host=${host}#${usr}-ws"
    local link_grpc="vless://${uuid}@${host}:443?security=tls&sni=${host}&type=grpc&serviceName=vless-grpc&mode=gun#${usr}-grpc"
    ...
    echo -e " Link TLS         : ${link_tls}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Link WS          : ${link_ws}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Link GRPC        : ${link_grpc}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Format OpenClash : https://${host}:81/vless-${usr}.txt"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Aktif Selama     : ${days} Hari"
    echo -e " Dibuat Pada      : ${created_fmt}"
    echo -e " Berakhir Pada    : ${exp_fmt}"
...
create_trojan() {
    ...
    echo "{\"username\":\"${usr}\",\"password\":\"${pass}\",\"created\":\"$(date +%Y-%m-%d)\",\"expired\":\"${exp}\",\"duration\":\"${days}\",\"quota\":\"${quota}\",\"maxip\":\"${maxip}\"}" > "$AKUN_DIR/trojan/${usr}.json"

    local host="$DOMAIN"
    local link_tls="trojan://${pass}@${host}:443?security=tls&sni=${host}&type=tcp#${usr}"
    local link_ws="trojan://${pass}@${host}:80?security=none&type=ws&path=/trojan&host=${host}#${usr}-ws"
    local link_grpc="trojan://${pass}@${host}:443?security=tls&sni=${host}&type=grpc&serviceName=trojan-grpc&mode=gun#${usr}-grpc"
    ...
    echo -e " Link GRPC        : ${link_grpc}"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Format OpenClash : https://${host}:81/trojan-${usr}.txt"
    echo -e "◇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◇"
    echo -e " Aktif Selama     : ${days} Hari"
    echo -e " Dibuat Pada      : ${created_fmt}"
    echo -e " Berakhir Pada    : ${exp_fmt}"
...
#!/usr/bin/env python3
import requests, subprocess, time, json, os, base64, uuid as _uuid, re
...
def create_vmess_link(usr, uid, host, port, path, tls, net='ws'):
    bug = 'bug.com'
    obj = {
        "v": "2", "ps": usr, "add": bug if tls == 'tls' else bug,
        "port": str(port), "id": uid, "aid": "0",
        "net": net, "path": path if net != 'grpc' else 'vmess-grpc',
        "type": "none", "host": host, "tls": tls
    }
    if net == 'grpc':
        obj['add'] = host; obj['host'] = bug
    return f"vmess://{b64(json.dumps(obj))}"

def handle_create_vmess(cid, parts):
    if len(parts) < 2:
        send(cid, '❌ Usage: /addvmess username [days] [quota_gb] [max_ip]')
        return
    usr = parts[1]
    days = int(parts[2]) if len(parts) > 2 else 30
    quota = parts[3] if len(parts) > 3 else '10000'
    maxip = parts[4] if len(parts) > 4 else '1'
    uid = gen_uuid()
    host = get_domain()
    exp = datetime.now() + timedelta(days=days)
    created = datetime.now()

    run(f"bash /root/tunnel.sh add-xray-cli vmess {uid} {usr}")

    os.makedirs(f'{AKUN_DIR}/vmess', exist_ok=True)
    data = json.dumps({"username": usr, "uuid": uid, "created": created.strftime('%Y-%m-%d'),
                       "expired": exp.strftime('%Y-%m-%d'), "duration": str(days),
                       "quota": quota, "maxip": maxip})
    with open(f'{AKUN_DIR}/vmess/{usr}.json', 'w') as f: f.write(data)

    link_tls  = create_vmess_link(usr, uid, host, 443, '/vmess', 'tls', 'ws')
    link_ntls = create_vmess_link(usr, uid, host, 80,  '/vmess', 'none', 'ws')
    link_grpc = create_vmess_link(usr, uid, host, 443, 'vmess-grpc', 'tls', 'grpc')
...
{link_grpc}
◇━━━━━━━━━━━━━━━━━━━━◇
Format OpenClash : https://{host}:81/vmess-{usr}.txt
◇━━━━━━━━━━━━━━━━━━━━◇
Aktif Selama  : {days} Hari
Dibuat Pada   : {fmt_date(created)}
Berakhir Pada : {fmt_date(exp)}
◇━━━━━━━━━━━━━━━━━━━━◇"""
    send(cid, msg, parse_mode='')

def handle_create_vless(cid, parts):
    if len(parts) < 2:
        send(cid, '❌ Usage: /addvless username [days] [quota_gb] [max_ip]')
        return
    usr = parts[1]
    days = int(parts[2]) if len(parts) > 2 else 30
    quota = parts[3] if len(parts) > 3 else '10000'
    ...
    run(f"bash /root/tunnel.sh add-xray-cli vless {uid} {usr}")

    os.makedirs(f'{AKUN_DIR}/vless', exist_ok=True)
    data = json.dumps({"username": usr, "uuid": uid, "created": created.strftime('%Y-%m-%d'),
                       "expired": exp.strftime('%Y-%m-%d'), "duration": str(days),
                       "quota": quota, "maxip": maxip})
    with open(f'{AKUN_DIR}/vless/{usr}.json', 'w') as f: f.write(data)

    link_tls  = f"vless://{uid}@{host}:443?security=tls&sni={host}&flow=xtls-rprx-vision&type=tcp#{usr}"
    link_ws   = f"vless://{uid}@{host}:80?security=none&type=ws&path=/vless&host={host}#{usr}-ws"
    link_grpc = f"vless://{uid}@{host}:443?security=tls&sni={host}&type=grpc&serviceName=vless-grpc&mode=gun#{usr}-grpc"
...
{link_grpc}
◇━━━━━━━━━━━━━━━━━━━━◇
Format OpenClash : https://{host}:81/vless-{usr}.txt
◇━━━━━━━━━━━━━━━━━━━━◇
Aktif Selama  : {days} Hari
Dibuat Pada   : {fmt_date(created)}
Berakhir Pada : {fmt_date(exp)}
...
def handle_create_trojan(cid, parts):
    if len(parts) < 2:
        send(cid, '❌ Usage: /addtrojan username [days] [quota_gb] [max_ip]')
        return
    usr = parts[1]
    days = int(parts[2]) if len(parts) > 2 else 30
    quota = parts[3] if len(parts) > 3 else '10000'
    maxip = parts[4] if len(parts) > 4 else '1'
    passwd = run(f"tr -dc 'a-z0-9' < /dev/urandom | head -c12").strip() or 'auto123'
    host = get_domain()
    exp = datetime.now() + timedelta(days=days)
    created = datetime.now()

    run(f"bash /root/tunnel.sh add-xray-cli trojan {passwd} {usr}")

    os.makedirs(f'{AKUN_DIR}/trojan', exist_ok=True)
    data = json.dumps({"username": usr, "password": passwd, "created": created.strftime('%Y-%m-%d'),
                       "expired": exp.strftime('%Y-%m-%d'), "duration": str(days),
                       "quota": quota, "maxip": maxip})
    with open(f'{AKUN_DIR}/trojan/{usr}.json', 'w') as f: f.write(data)

    link_tls  = f"trojan://{passwd}@{host}:443?security=tls&sni={host}&type=tcp#{usr}"
    link_ws   = f"trojan://{passwd}@{host}:80?security=none&type=ws&path=/trojan&host={host}#{usr}-ws"
    link_grpc = f"trojan://{passwd}@{host}:443?security=tls&sni={host}&type=grpc&serviceName=trojan-grpc&mode=gun#{usr}-grpc"
...
{link_grpc}
◇━━━━━━━━━━━━━━━━━━━━◇
Format OpenClash : https://{host}:81/trojan-{usr}.txt
◇━━━━━━━━━━━━━━━━━━━━◇
Aktif Selama  : {days} Hari
Dibuat Pada   : {fmt_date(created)}
Berakhir Pada : {fmt_date(exp)}
...
def handle_create_ssh(cid, parts):
    if len(parts) < 2:
        send(cid, '❌ Usage: /addssh username [days] [max_ip]')
        return
    usr = parts[1]
    days = int(parts[2]) if len(parts) > 2 else 30
    maxip = parts[3] if len(parts) > 3 else '2'
    passwd = run(f"tr -dc 'a-zA-Z0-9' < /dev/urandom | head -c10").strip() or 'pass123'
    host = get_domain()
    exp = datetime.now() + timedelta(days=days)
    exp_date = exp.strftime('%Y-%m-%d')
    created = datetime.now()

    run(f"useradd -M -s /bin/false -e {exp_date} {usr} 2>/dev/null")
    run(f"echo '{usr}:{passwd}' | chpasswd 2>/dev/null")

    os.makedirs(f'{AKUN_DIR}/ssh', exist_ok=True)
    data = json.dumps({"username": usr, "password": passwd, "created": created.strftime('%Y-%m-%d'),
                       "expired": exp_date, "maxlogin": maxip, "duration": str(days)})
    with open(f'{AKUN_DIR}/ssh/{usr}.json', 'w') as f: f.write(data)
...
def get_account_list():
    host = get_domain()
    lines = [f'<b>📋 Account List — {host}</b>\n']
    for proto in ['vmess', 'vless', 'trojan', 'ssh']:
        d = f'{AKUN_DIR}/{proto}'
        if not os.path.isdir(d): continue
        files = [f for f in os.listdir(d) if f.endswith('.json')]
        if not files: continue
        lines.append(f'\n<b>── {proto.upper()} ({len(files)}) ──</b>')
        for fn in sorted(files):
            try:
                dat = json.load(open(f'{d}/{fn}'))
                usr = dat.get('username', fn.replace('.json', ''))
                exp = dat.get('expired', 'N/A')
                lines.append(f'  • {usr} → {exp}')
            except: pass
    return '\n'.join(lines)
...
def handle(msg):
    ...
    if cmd == '/start':
        send(cid, """<b>👋 Welcome to YOUZINCRABZ VPN Bot!</b>

<b>📦 Account Commands:</b>
/addvmess user [days] [gb] [ip]
/addvless user [days] [gb] [ip]
/addtrojan user [days] [gb] [ip]
/addssh user [days] [ip]
...
<b>📋 List Commands:</b>
/listall — Show all accounts
/listvmess — VMess accounts
/listvless — VLess accounts
/listtrojan — Trojan accounts
/listssh — SSH accounts

<b>ℹ️ Server Commands:</b>
/status — Service status
/info — Server info
/restart — Restart all services
/speedtest — Speed test""")

    elif cmd == '/addvmess': handle_create_vmess(cid, parts)
    elif cmd == '/addvless': handle_create_vless(cid, parts)
    elif cmd == '/addtrojan': handle_create_trojan(cid, parts)
    elif cmd == '/addssh': handle_create_ssh(cid, parts)

    elif cmd in ('/delvmess', '/delvless', '/deltrojan', '/delssh'):
        proto = cmd.replace('/del', '')
        if len(parts) < 2: send(cid, f'Usage: {cmd} username'); return
        usr = parts[1]
        if proto == 'ssh':
            run(f"userdel -f {usr} 2>/dev/null")
        else:
            run(f"bash /root/tunnel.sh del-xray-cli {proto} {usr}")
        os.remove(f'{AKUN_DIR}/{proto}/{usr}.json') if os.path.exists(f'{AKUN_DIR}/{proto}/{usr}.json') else None
        send(cid, f'✅ {usr} ({proto.upper()}) deleted!')

    elif cmd == '/listall': send(cid, get_account_list())
    elif cmd.startswith('/list'):
        proto = cmd.replace('/list', '') or 'all'
        if proto == 'all': send(cid, get_account_list()); return
        d = f'{AKUN_DIR}/{proto}'
        if not os.path.isdir(d): send(cid, f'No {proto} accounts'); return
        files = [f for f in os.listdir(d) if f.endswith('.json')]
        if not files: send(cid, f'No {proto.upper()} accounts found'); return
        lines = [f'<b>── {proto.upper()} ACCOUNTS ──</b>']
        for fn in sorted(files):
            try:
                dat = json.load(open(f'{d}/{fn}'))
                usr = dat.get('username', fn.replace('.json',''))
                exp = dat.get('expired','N/A')
                lines.append(f'• {usr} → Exp: {exp}')
            except: pass
        send(cid, '\n'.join(lines))

    elif cmd == '/status':
        svcs = ['xray','nginx','haproxy','dropbear','stunnel4','sshd','udp-custom','vpn-bot']
        lines = ['<b>🔧 Service Status:</b>']
        for s in svcs:
            r = run(f"systemctl is-active {s} 2>/dev/null")
            icon = '🟢' if r.strip() == 'active' else '🔴'
            lines.append(f'{icon} {s}: {r.strip()}')
        send(cid, '\n'.join(lines))

    elif cmd == '/info':
        ip = run("curl -s --max-time 5 https://ifconfig.me 2>/dev/null || echo N/A")
        up = run("uptime -p")
        host = get_domain()
        cpu = run("top -bn1 | grep 'Cpu' | awk '{print $2}' | cut -d'%' -f1")
        ram = run("free -m | awk 'NR==2{print $3\"/\"$2\" MB\"}'")
        send(cid, f"""<b>ℹ️ Server Info</b>
🌐 IP      : <code>{ip}</code>
🔗 Domain  : <code>{host}</code>
⏱ Uptime  : {up}
💻 CPU     : {cpu}%
🧠 RAM     : {ram}""")

    elif cmd == '/restart':
        run("systemctl restart xray nginx haproxy dropbear stunnel4 2>/dev/null")
        send(cid, '✅ Services restarted!')

    elif cmd == '/speedtest':
        send(cid, '⏳ Running speedtest...')
        result = run("speedtest --accept-license --accept-gdpr 2>/dev/null | grep -E 'Download|Upload|Latency'")
        send(cid, f'<b>🚀 Speedtest Result:</b>\n<code>{result}</code>')

    else:
        send(cid, '❓ Unknown command. Send /start for help.')
...
menu_restart() {
    ...
    for svc in xray nginx haproxy dropbear stunnel4 sshd udp-custom vpn-keepalive vpn-bot; do
...
#================================================
# STUNNEL4 SETUP & MANAGEMENT
#================================================
setup_stunnel4() {
    apt-get install -y stunnel4 -qq 2>/dev/null || { fail_msg "stunnel4 install failed"; return 1; }
    
    # Generate self-signed cert if xray cert not available
    local cert_file="/etc/xray/xray.crt"
    local key_file="/etc/xray/xray.key"
    [[ ! -f "$cert_file" ]] && { _gen_self_signed; cert_file="/etc/xray/xray.crt"; key_file="/etc/xray/xray.key"; }

    # Combine cert+key for stunnel
    cat "$cert_file" "$key_file" > /etc/stunnel/stunnel.pem
    chmod 600 /etc/stunnel/stunnel.pem

    cat > /etc/stunnel/stunnel.conf << STUNCFG
; Stunnel4 Configuration — YouzinCrabz Panel
...
[ssh-ssl]
accept  = ${STUNNEL_SSH_PORT}
connect = 127.0.0.1:22
ciphers = TLSv1.2

[dropbear-ssl]
accept  = ${STUNNEL_DROP_PORT}
connect = 127.0.0.1:222
ciphers = TLSv1.2
STUNCFG

    # Enable stunnel4 autostart
...
menu_stunnel4() {
    while true; do
        clear
        local st_stun=$(check_status "stunnel4")
        _box_top; _box_center "STUNNEL4 MANAGEMENT"; _box_mid
        _box_row "${WHITE}Status        :${NC} $([ "$st_stun" == "ON" ] && echo "${GREEN}● ONLINE${NC}" || echo "${RED}○ OFFLINE${NC}")" 30
        _box_row "${WHITE}SSH over SSL  :${NC} Port ${STUNNEL_SSH_PORT}" $(( 16 + ${#STUNNEL_SSH_PORT} ))
        _box_row "${WHITE}Drop over SSL :${NC} Port ${STUNNEL_DROP_PORT}" $(( 16 + ${#STUNNEL_DROP_PORT} ))
        _box_mid
        _box_2col "${CYAN}[1]${NC} Install/Reconfigure" "${CYAN}[3]${NC} Restart"  $(( 3+18 )) $(( 3+7 ))
        _box_2col "${CYAN}[2]${NC} Show Config"         "${CYAN}[4]${NC} Stop"     $(( 3+11 )) $(( 3+4 ))
        _box_2col "${CYAN}[5]${NC} Change Ports"        "${CYAN}[0]${NC} Back"     $(( 3+12 )) $(( 3+4 ))
        _box_bot; echo ""
        read -p " Select [0-5]: " ch
        case $ch in
            1)
                clear; echo -e "${CYAN}Setting up Stunnel4...${NC}"
                setup_stunnel4 && echo -e "${GREEN}Stunnel4 configured!${NC}" || echo -e "${RED}Setup failed!${NC}"
                sleep 3 ;;
            2)
                clear; _box_top; _box_center "STUNNEL4 CONFIG"; _box_mid
                if [[ -f /etc/stunnel/stunnel.conf ]]; then
                    while IFS= read -r line; do
                        _box_row "${line}" ${#line}
                    done < <(grep -v "^;" /etc/stunnel/stunnel.conf | grep -v "^$" | head -20)
                else
                    _box_center "Config not found!" "$RED"
                fi
                _box_bot; echo ""; read -p " Press any key..." ;;
            3) systemctl restart stunnel4 2>/dev/null && echo -e "${GREEN}Stunnel4 restarted!${NC}" || echo -e "${RED}Failed!${NC}"; sleep 2 ;;
            4) systemctl stop stunnel4 2>/dev/null && echo -e "${YELLOW}Stunnel4 stopped!${NC}" || echo -e "${RED}Failed!${NC}"; sleep 2 ;;
            5)
                read -p " SSH over SSL port [${STUNNEL_SSH_PORT}]: " p1
                read -p " Dropbear over SSL port [${STUNNEL_DROP_PORT}]: " p2
                [[ -n "$p1" ]] && STUNNEL_SSH_PORT="$p1"
                [[ -n "$p2" ]] && STUNNEL_DROP_PORT="$p2"
                setup_stunnel4
                echo -e "${GREEN}Ports updated!${NC}"; sleep 2 ;;
            0) return ;;
        esac
    done
}
...
auto_install() {
    ...
    info_msg "Configuring Stunnel4..."; setup_stunnel4 && done_msg "Stunnel4 configured" || fail_msg "Stunnel4 failed"
    ...
    _box_row "${WHITE}Stunnel:${NC} SSH-SSL Port ${STUNNEL_SSH_PORT} / Drop-SSL Port ${STUNNEL_DROP_PORT}" $(( 10 + ${#STUNNEL_SSH_PORT} + 16 + ${#STUNNEL_DROP_PORT} ))
