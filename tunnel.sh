...
ASCIIEOF
    echo -e "${NC}"
    echo -e "${WHITE}        AUTO VPN SCRIPT v${SCRIPT_VERSION}${NC}"
...
_uninstall_all() {
    ...
    bash <(curl -Ls \
        https://github.com/XTLS/Xray-install/raw/main/install-release.sh) \
        --remove >/dev/null 2>&1
    apt-get purge -y nginx haproxy dropbear \
        >/dev/null 2>&1
    apt-get autoremove -y >/dev/null 2>&1
    rm -rf /usr/local/etc/xray \
           /var/log/xray /etc/xray \
           /root/akun /root/bot \
           /root/orders /root/domain \
           /root/.domain_type \
           /root/.bot_token \
           /root/.chat_id \
           /root/.payment_info \
...
show_system_info() {
    ...
    local HL="========================================================"
    echo -e "${CYAN}+${HL}+${NC}"
    printf "${CYAN}|${NC}%*s${GREEN}Welcome Mr. %-14s${NC}%*s${CYAN}|${NC}\n" \
        9 "" "$USERNAME" 9 ""
    echo -e "${CYAN}+${HL}+${NC}"
    printf "${CYAN}|${NC} ${WHITE}*${NC} %-15s = ${GREEN}%-32s${NC}${CYAN}|${NC}\n" \
        "SYSTEM OS"   "$os_name"
    printf "${CYAN}|${NC} ${WHITE}*${NC} %-15s = ${GREEN}%-32s${NC}${CYAN}|${NC}\n" \
    ...
    local IL="==============================================="
    echo ""
    printf "%*s${CYAN}>>> INFORMATION ACCOUNT <<<${NC}\n" \
        17 ""
    printf "%*s${CYAN}+${IL}+${NC}\n" 6 ""
    printf "%*s${CYAN}|${NC} %-20s = ${GREEN}%-22s${NC} ${CYAN}|${NC}\n" \
        6 "" "SSH/UDP"        "$sc"
    printf "%*s${CYAN}|${NC} %-20s = ${GREEN}%-22s${NC} ${CYAN}|${NC}\n" \
        6 "" "VMESS/WS/GRPC"  "$vc"
    printf "%*s${CYAN}|${NC} %-20s = ${GREEN}%-22s${NC} ${CYAN}|${NC}\n" \
    ...
    # Box status - lebar tiap kolom 18 char
    # format: | LABEL   STATUS  |
    # label=6, spasi=3, status=3, spasi=4 = 16
    local SEP="${CYAN}+================+================+================+${NC}"

    echo -e "$SEP"
    printf "${CYAN}|${NC} %-6s %b  ${CYAN}|${NC} %-6s %b  ${CYAN}|${NC} %-6s %b  ${CYAN}|${NC}\n" \
        "SSH"    "$c1" \
        "NGINX"  "$c2" \
        "XRAY"   "$c3"
    printf "${CYAN}|${NC} %-6s %b  ${CYAN}|${NC} %-6s %b  ${CYAN}|${NC} %-6s %b  ${CYAN}|${NC}\n" \
        ...
        "DROPBR" "$c6" \
        "UDP"    "$c4"
    printf "${CYAN}|${NC} %-6s %b  ${CYAN}|${NC} %-6s %b  ${CYAN}|${NC}                 ${CYAN}|${NC}\n" \
        "PINGKA" "$c7" \
        "BOT"    "$c8"
    echo -e "$SEP"
    ...
    printf "%*s${CYAN}┌───────────────────────────────────┐${NC}\n" \
        10 ""
    printf "%*s${CYAN}│${NC}  📞 Kontak Admin : ${WHITE}@ridhani16${NC}       ${CYAN}│${NC}\n" \
        10 ""
    printf "%*s${CYAN}│${NC}  🌐 Telegram     : ${WHITE}t.me/ridhani16${NC}   ${CYAN}│${NC}\n" \
        10 ""
    printf "%*s${CYAN}└───────────────────────────────────┘${NC}\n" \
        10 ""
    echo ""
...
show_menu() {
    local ML="========================================================"
    echo -e "${CYAN}+${ML}+${NC}"
    printf "${CYAN}|${NC} ${WHITE}[01]${NC} SSH MENU    ${CYAN}|${NC} ${WHITE}[07]${NC} SWAP SETUP  ${CYAN}|${NC} ${WHITE}[13]${NC} TELE BOT   ${CYAN}|${NC}\n"
    printf "${CYAN}|${NC} ${WHITE}[02]${NC} VMESS MENU  ${CYAN}|${NC} ${WHITE}[08]${NC} OPTIMIZE    ${CYAN}|${NC} ${WHITE}[14]${NC} CHG DOMAIN ${CYAN}|${NC}\n"
    printf "${CYAN}|${NC} ${WHITE}[03]${NC} VLESS MENU  ${CYAN}|${NC} ${WHITE}[09]${NC} RESTART ALL ${CYAN}|${NC} ${WHITE}[15]${NC} FIX CERT   ${CYAN}|${NC}\n"
    printf "${CYAN}|${NC} ${WHITE}[04]${NC} TROJAN MENU ${CYAN}|${NC} ${WHITE}[10]${NC} RUNNING     ${CYAN}|${NC} ${WHITE}[16]${NC} UPDATE     ${CYAN}|${NC}\n"
    printf "${CYAN}|${NC} ${WHITE}[05]${NC} CEK EXPIRED ${CYAN}|${NC} ${WHITE}[11]${NC} INFO PORT   ${CYAN}|${NC} ${WHITE}[17]${NC} SPEEDTEST  ${CYAN}|${NC}\n"
    printf "${CYAN}|${NC} ${WHITE}[06]${NC} DEL EXPIRED ${CYAN}|${NC} ${WHITE}[12]${NC} INSTALL UDP ${CYAN}|${NC} ${WHITE}[18]${NC} UNINSTALL  ${CYAN}|${NC}\n"
    printf "${CYAN}|${NC}                 ${CYAN}|${NC}                 ${CYAN}|${NC} ${WHITE}[00]${NC} EXIT       ${CYAN}|${NC}\n"
    echo -e "${CYAN}+${ML}+${NC}"
    printf "${CYAN}|${NC} Ver: ${GREEN}%-8s${NC} | Author: ${GREEN}%-31s${NC}${CYAN}|${NC}\n" \
...
show_info_port() {
    ...
    printf "${CYAN}|${NC}%*s${WHITE}SERVER PORT INFORMATION${NC}%*s${CYAN}|${NC}\n" \
        19 "" 18 ""
    echo -e "${CYAN}+${HL}+${NC}"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-26s${NC}${CYAN}|${NC}\n" \
        "SSH"              "22"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-26s${NC}${CYAN}|${NC}\n" \
        "Dropbear"         "222"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-26s${NC}${CYAN}|${NC}\n" \
    ...
    try:
        if protocol == 'ssh':
            ...
            msg = fmt_xray_msg(
                ...
                link_grpc,
                f'Akun {protocol.upper()} Berhasil! ✅'
            )
        msg += '\n💰 Terima kasih! 🙏'
        send(chat_id, msg, markup=kb_main())
        return True, msg
...
def on_msg(msg):
    ...
    if text in ['/start','🏠 Menu']:
        ...
        orders = get_pending()
        if not orders:
            send(ADMIN_ID,'📭 Tidak ada pending.')
            return
        pay   = get_payment()
        harga = int(pay.get('HARGA',10000))
        for o in orders[:5]:
...
# ── Main Loop ─────────────────────────────────
def main():
    print(f'Bot VPN - Admin: {ADMIN_ID}',
          flush=True)
    offset = 0
    pool   = []
    while True:
...
auto_install() {
    ...
    # Install package dengan spinner
    _pkg() {
        local pkg="$1"
        local spin=('⠋' '⠙' '⠹' '⠸' '⠼' \
                    '⠴' '⠦' '⠧' '⠇' '⠏')
