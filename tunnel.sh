...
# PORT VARIABLES
# 80   -> Nginx (webserver)
# 443  -> HAProxy (loadbalancer/SSL termination)
...
setup_swap() {
    ...
    if [[ "$swap_total" -gt 0 ]]; then
        echo -e "${YELLOW}Swap already exists: ${swap_total}MB${NC}"
        echo -e "${YELLOW}Recreating swap...${NC}"
        swapoff -a
        sed -i '/swapfile/d' /etc/fstab
        rm -f /swapfile
    fi

    echo -e "${CYAN}Creating 1GB swap file...${NC}"
    ...
    local swap_now
    swap_now=$(free -h | awk 'NR==3{print $2}')
    echo ""
    echo -e "${GREEN}Swap 1GB created! Total swap: ${swap_now}${NC}"
    sleep 2
}

# ================================================
# OPTIMASI VPN - PING STABIL
# ================================================

optimize_vpn() {
    echo -e "${CYAN}Optimizing VPN network settings...${NC}"

    # Sysctl tuning lengkap untuk VPN stabil
    cat > /etc/sysctl.d/99-vpn-optimize.conf << 'SYSCTLEOF'
# TCP Keepalive - jaga koneksi tetap hidup
...
# TCP Buffer sizes - throughput stabil
...
# BBR congestion control - mengurangi packet loss
...
# Disable slow start after idle - PENTING untuk VPN
net.ipv4.tcp_slow_start_after_idle = 0

# IP forwarding untuk VPN
...
SYSCTLEOF
    ...
    # Optimize ulimit
    cat > /etc/security/limits.d/99-vpn.conf << 'LIMITSEOF'
* soft nofile 65535
* hard nofile 65535
* soft nproc 65535
* hard nproc 65535
...
SDEOF
    ...
    echo -e "${GREEN}VPN optimization complete!${NC}"
}

# ================================================
# SETUP KEEPALIVE - PING STABIL
# ================================================
...
XKEOF
    ...
    # VPN Keepalive script - kirim ping tiap 25 detik
    cat > /usr/local/bin/vpn-keepalive.sh << 'VPNEOF'
#!/bin/bash
# VPN Keepalive - cegah idle disconnect & NAT expiry
...
while true; do
    ...
    # Ping DNS Google untuk jaga routing
    ping -c 1 -W 2 8.8.8.8 >/dev/null 2>&1

    # UDP keepalive ke BadVPN ports
...
[Unit]
Description=VPN Keepalive - Stabilize Ping
...
fix_certificate() {
    ...
    # Stop haproxy & nginx untuk bebaskan port 80
    echo -e "${CYAN}Stopping services for port 80...${NC}"
    ...
    echo ""
    echo -e "${GREEN}Done! All services restarted.${NC}"
    sleep 3
}
...
run_speedtest() {
    ...
    if command -v speedtest >/dev/null 2>&1; then
        ...
        if command -v speedtest-cli >/dev/null 2>&1; then
            ...
            command -v speedtest >/dev/null 2>&1 && speedtest || \
                echo -e "${RED}Speedtest install failed. Try: pip3 install speedtest-cli${NC}"
        fi
    fi
...
#================================================
# CREATE XRAY CONFIG
# 8443 -> VMess/VLess TLS WS
# 8080 -> VMess/VLess NonTLS WS
# 2083 -> Trojan TLS WS
# 8444 -> gRPC semua protocol
#================================================
...
show_system_info() {
    ...
    local ip_vps ram cpu uptime_str swap_info
...
#================================================
# INSTALL UDP CUSTOM - PORT 7100-7300
#================================================
...
server {
    ...
    # WebSocket proxy untuk Xray NonTLS
    location /vmess {
...
NGXEOF
    ...
    # Hapus default nginx sites yang conflict
...
DBEOF
    ...
    echo -e "${CYAN}[8/9]${NC} Configuring HAProxy (port 443 -> Xray 8443)..."
...
HAEOF
    ...
    if ! grep -q "tunnel.sh" /root/.bashrc; then
        echo "" >> /root/.bashrc
        echo "[[ -f /root/tunnel.sh ]] && bash /root/tunnel.sh" >> /root/.bashrc
    fi
    ...
    sleep 2
    ss -tulpn | grep -E ':22|:80|:222|:443|:8080|:8443|:2083|:8444' | \
        awk '{print $1, $5, $7}' | \
        while read proto addr proc; do
            echo -e " ${GREEN}+${NC} $proto $addr $proc"
        done
    ...
    while true; do
        ...
        case $choice in
            ...
            13)
                ...
                ss -tulpn | grep -E \
                    ':22 |:80 |:222 |:443 |:8080 |:8443 |:2083 |:8444 ' \
                    2>/dev/null | awk '{print $1, $5}' | \
                    while read p a; do
                        echo -e " ${GREEN}+${NC} $p $a"
                    done
                echo ""
