...
SSH_PORTS="443, 80, 22"
DROPBEAR_PORTS="443, 109, 143"
DROPBEAR_WS_PORTS="443, 109"
SSH_WS_SSL_PORT="443"
SSH_WS_PORT="80"
SSH_UDP_PORTS="1-65535"
OPENVPN_SSL_PORT="443"
OPENVPN_WS_SSL_PORT="443"
OPENVPN_TCP_PORTS="443, 1194"
OPENVPN_UDP_PORT="2200"
NGINX_PORTS="443, 80, 81"
HAPROXY_PORTS="443, 80"
DNS_SERVER_PORTS="443, 53"
DNS_CLIENT_PORTS="443, 88"
SLOWDNS_PORTS="443, 53"
VMESS_TLS_PORT="443"
VMESS_GRPC_PORT="443"
VMESS_NONTLS_PORT="80"
VLESS_TLS_PORT="443"
VLESS_GRPC_PORT="443"
VLESS_NONTLS_PORT="80"
TROJAN_GRPC_PORT="443"
TROJAN_WS_PORT="443"
SHADOWSOCKS_WS_PORT="443"
BADVPN_PORT1="7100"
BADVPN_PORT2="7200"
BADVPN_PORT3="7300"
...
while true; do
    ...
    echo -n "ka" | nc -u -w1 127.0.0.1 7300 >/dev/null 2>&1
    sleep 30
done
VPNEOF
...
show_system_info() {
    ...
    # Build colored string — echo -e tiap baris agar warna render
...
show_info_port() {
    ...
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "Dropbear Websocket"    "$DROPBEAR_WS_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "SSH Websocket SSL"     "$SSH_WS_SSL_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "SSH Websocket"         "$SSH_WS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "SSH UDP"               "$SSH_UDP_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "OpenVPN SSL"           "$OPENVPN_SSL_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "OpenVPN Websocket SSL" "$OPENVPN_WS_SSL_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "OpenVPN TCP"           "$OPENVPN_TCP_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "OpenVPN UDP"           "$OPENVPN_UDP_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "Nginx Webserver"       "$NGINX_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "Haproxy Loadbalancer"  "$HAPROXY_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "DNS Server"            "$DNS_SERVER_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "DNS Client"            "$DNS_CLIENT_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY DNS SLOWDNS"      "$SLOWDNS_PORTS"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY Vmess TLS"        "$VMESS_TLS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY Vmess gRPC"       "$VMESS_GRPC_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY Vmess None TLS"   "$VMESS_NONTLS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY Vless TLS"        "$VLESS_TLS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY Vless gRPC"       "$VLESS_GRPC_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "XRAY Vless None TLS"   "$VLESS_NONTLS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "Trojan gRPC"           "$TROJAN_GRPC_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "Trojan WS"             "$TROJAN_WS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "Shadowsocks WS"        "$SHADOWSOCKS_WS_PORT"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "BadVPN 1"              "$BADVPN_PORT1"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "BadVPN 2"              "$BADVPN_PORT2"
    printf "${CYAN}|${NC} %-28s : ${GREEN}%-20s${NC} ${CYAN}|${NC}\n" "BadVPN 3"              "$BADVPN_PORT3"
    echo -e "${CYAN}+========================================================+${NC}"
    echo ""
    read -p "Press Enter..."
}
...
create_account_template() {
    ...
    if [[ "$protocol" == "vmess" ]]; then
        ...
        j_grpc=$(printf '{"v":"2","ps":"%s","add":"%s","port":"443","id":"%s","aid":"0","net":"grpc","path":"vmess-grpc","type":"none","host":"bug.com","tls":"tls"}' \
        ...
        link_nontls="vless://${uuid}@bug.com:80?path=%2Fvless&security=none&encryption=none&host=${DOMAIN}&type=ws#${username}"
        link_grpc="vless://${uuid}@${DOMAIN}:443?mode=gun&security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=bug.com#${username}"

    elif [[ "$protocol" == "trojan" ]]; then
        link_tls="trojan://${uuid}@bug.com:443?path=%2Ftrojan&security=tls&host=${DOMAIN}&type=ws&sni=${DOMAIN}#${username}"
        link_nontls="trojan://${uuid}@bug.com:80?path=%2Ftrojan&security=none&host=${DOMAIN}&type=ws#${username}"
        link_grpc="trojan://${uuid}@${DOMAIN}:443?mode=gun&security=tls&type=grpc&serviceName=trojan-grpc&sni=bug.com#${username}"
    fi

    mkdir -p "$PUBLIC_HTML"
    printf "proxies:\n  - name: \"%s\"\n    type: %s\n    server: %s\n    port: 443\n    uuid: %s\n    alterId: 0\n    cipher: auto\n    tls: true\n    network: ws\n    ws-opts:\n      path: /%s\n      headers:\n        Host: %s\n" \
        "$username" "$protocol" "$DOMAIN" "$uuid" "$protocol" "$DOMAIN" \
    ...
    case $protocol in
        vmess)
            ...
            printf " %-16s : %s\n" "Port none TLS" "$VMESS_NONTLS_PORT"
            printf " %-16s : %s\n" "id"            "$uuid"
            printf " %-16s : %s\n" "alterId"       "0"
            printf " %-16s : %s\n" "Security"      "auto"
            printf " %-16s : %s\n" "Network"       "ws"
        ...
        vless)
            ...
            printf " %-16s : %s\n" "Port none TLS" "$VLESS_NONTLS_PORT"
            printf " %-16s : %s\n" "id"            "$uuid"
            printf " %-16s : %s\n" "Encryption"    "none"
            printf " %-16s : %s\n" "Network"       "ws"
            printf " %-16s : %s\n" "Path"          "/vless"
            ...
            ;;
        trojan)
            printf " %-16s : %s\n" "Port WS TLS"   "$TROJAN_WS_PORT"
            printf " %-16s : %s\n" "Port gRPC"     "$TROJAN_GRPC_PORT"
            printf " %-16s : %s\n" "Password"      "$uuid"
            printf " %-16s : %s\n" "Network"       "ws"
            printf " %-16s : %s\n" "Path"          "/trojan"
            printf " %-16s : %s\n" "Dynamic"       "https://bugmu.com/path"
    ...
    printf " %-16s : %s\n" "Link TLS"      "$link_tls"
    echo -e "${CYAN}+=========================================+${NC}"
    printf " %-16s : %s\n" "Link none TLS" "$link_nontls"
    echo -e "${CYAN}+=========================================+${NC}"
    printf " %-16s : %s\n" "Link GRPC"     "$link_grpc"
    echo -e "${CYAN}+=========================================+${NC}"
    printf " %-16s : https://%s:81/%s-%s.txt\n" "Format OpenClash" "$DOMAIN" "$protocol" "$username"
    echo -e "${CYAN}+=========================================+${NC}"
    printf " %-16s : %s Hari\n" "Aktif Selama"  "$days"
    printf " %-16s : %s\n"      "Dibuat Pada"   "$created"
    printf " %-16s : %s\n"      "Berakhir Pada" "$exp"
...
delete_account() {
    ...
    jq --arg email "$username" \
       ...
       "$XRAY_CONFIG" > "$temp" 2>/dev/null && mv "$temp" "$XRAY_CONFIG" || rm -f "$temp"
    ...
    if [[ "$protocol" == "ssh" ]]; then
        ...
        if [[ -f /var/log/xray/access.log ]]; then
            grep -i "$protocol" /var/log/xray/access.log 2>/dev/null | \
                awk '{print $NF}' | sort | uniq -c | sort -rn | head -20 || echo "No data"
        else
            echo "No log data"
        fi
    fi
...
install_udp_custom() {
    clear
    echo -e "${CYAN}Installing UDP Custom (BadVPN 7100, 7200, 7300)...${NC}"
...
[Unit]
Description=UDP Custom BadVPN 7100 7200 7300
After=network.target
...
UDPSVCEOF
    ...
    echo -e "${GREEN}UDP Custom installed! Ports: 7100, 7200, 7300${NC}"
    sleep 2
}
#================================================
# AUTO INSTALL
...
auto_install() {
    ...
    if [[ -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]]; then
        ...
        openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 \
            ...
            -out /etc/xray/xray.crt >/dev/null 2>&1
    fi
    ...
    echo -e "${CYAN}[5/8]${NC} Configuring Nginx..."
...
NGXEOF
    ...
    echo -e "${CYAN}[6/8]${NC} Configuring Dropbear (443, 109, 143)..."
    cat > /etc/default/dropbear << 'DBEOF'
NO_START=0
DROPBEAR_PORT=443
DROPBEAR_EXTRA_ARGS="-p 109 -p 143 -K 60 -I 180"
DROPBEAR_BANNER="/etc/issue.net"
DROPBEAR_RECEIVE_WINDOW=65536
DBEOF

    echo -e "${CYAN}[7/8]${NC} Configuring HAProxy (443, 80)..."
...
HAEOF
    ...
    echo -e "${CYAN}[8/8]${NC} Installing UDP & Keepalive..."
