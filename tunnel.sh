...
show_install_banner() {
    ...
    echo -e "${CYAN}  ║${NC}   ${YELLOW}   ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝╚═╝╚═╝  ╚══╝${NC}  ${CYAN}║${NC}"
    echo -e "${CYAN}  ║${NC}                                                  ${CYAN}║${NC}"
    echo -e "${CYAN}  ║${NC}         ${GREEN}C R A B Z   T U N E L${NC}                  ${CYAN}║${NC}"
    echo -e "${CYAN}  ║${NC}         ${WHITE}Powered by ${YELLOW}The Professor${NC}                  ${CYAN}║${NC}"
    echo -e "${CYAN}  ║${NC}         ${DIM}v${SCRIPT_VERSION} · Ultimate VPN Script${NC}                ${CYAN}║${NC}"
    echo -e "${CYAN}  ║${NC}                                                  ${CYAN}║${NC}"
    echo -e "${CYAN}  ╚══════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
show_system_info() {
    ...
    if [[ "$domain_type" == "custom" ]]; then
        ...
        if [[ -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]]; then
            ...
            ssl_status="${YELLOW}Warning${NC}"
        fi
    else
        ssl_type="Self-Signed"
        ssl_status="${CYAN}Self-Sign${NC}"
    fi
    ...
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════╗${NC}"
    printf "${CYAN}║${NC}  ${YELLOW}★${NC} ${WHITE}YOUZIN CRABZ TUNEL${NC} ${DIM}v${SCRIPT_VERSION}${NC}%*s${CYAN}║${NC}\n" 33 ""
    printf "${CYAN}║${NC}     ${DIM}The Professor${NC}%*s${CYAN}║${NC}\n" 51 ""
    echo -e "${CYAN}╠══════════════════════════════════════════════════════════════════╣${NC}"
    printf "${CYAN}║${NC}  ${WHITE}IP VPS   :${NC} ${GREEN}%-18s${NC}  ${WHITE}Domain  :${NC} ${CYAN}%-18s${NC}  ${CYAN}║${NC}\n" \
        "$ip_vps" "${DOMAIN:-Not Set}"
    printf "${CYAN}║${NC}  ${WHITE}Uptime   :${NC} %-18s  ${WHITE}OS      :${NC} %-18s  ${CYAN}║${NC}\n" \
        "$uptime_str" "${os_name:0:18}"
    printf "${CYAN}║${NC}  ${WHITE}CPU      :${NC} ${YELLOW}%-3s%%%${NC}               ${WHITE}RAM     :${NC} ${YELLOW}[${ram_bar}]${NC} ${ram_pct}%%      ${CYAN}║${NC}\n" \
        "$cpu"
    printf "${CYAN}║${NC}  ${WHITE}SSL      :${NC} %-14b        ${WHITE}Services:${NC} ${GREEN}%d${NC}/${svc_total} Running              ${CYAN}║${NC}\n" \
        "$ssl_type $ssl_status" "$svc_running"
    echo -e "${CYAN}╠══════════════════════════════════════════════════════════════════╣${NC}"
    printf "${CYAN}║${NC}  ${WHITE}SSH:${NC} ${GREEN}%2d${NC} akun  ${WHITE}VMess:${NC} ${GREEN}%2d${NC} akun  ${WHITE}VLess:${NC} ${GREEN}%2d${NC} akun  ${WHITE}Trojan:${NC} ${GREEN}%2d${NC} akun  ${CYAN}║${NC}\n" \
    ...
    _svc_status_inline() {
        ...
        if systemctl is-active --quiet "$svc" 2>/dev/null; then
            printf "${CYAN}║${NC}  ${GREEN}●${NC} %-14s ${GREEN}ON ${NC}" "$label"
        else
            printf "${CYAN}║${NC}  ${RED}○${NC} %-14s ${RED}OFF${NC}" "$label"
        fi
    }

    _svc_status_inline "xray"          "Xray"
    printf "    "
    _svc_status_inline "haproxy"       "HAProxy"
    printf "    "
    _svc_status_inline "nginx"         "Nginx"
    printf "\n"
    _svc_status_inline "dropbear"      "Dropbear"
    printf "    "
    _svc_status_inline "sshd"          "SSH"
    printf "    "
    _svc_status_inline "udp-custom"    "UDP Custom"
    printf "\n"
    _svc_status_inline "vpn-bot"       "Bot Telegram"
    printf "    "
    _svc_status_inline "vpn-keepalive" "Keepalive"
    printf "%*s${CYAN}║${NC}\n" 14 ""
...
show_menu() {
    ...
    echo -e "${CYAN}║${NC}   ${CYAN}[2]${NC} VMess               ${CYAN}[5]${NC} Trial Xray   ${CYAN}[8]${NC} Delete Expired  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${CYAN}[3]${NC} VLess               ${CYAN}[6]${NC} List Semua                    ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}                                                                  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}  ${YELLOW}─── SYSTEM & TOOLS ──────────────────────────────────────────${NC}  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${CYAN}[9]${NC}  Telegram Bot       ${CYAN}[13]${NC} Restart All  ${CYAN}[17]${NC} Backup         ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${CYAN}[10]${NC} Change Domain      ${CYAN}[14]${NC} Info Port    ${CYAN}[18]${NC} Restore        ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${CYAN}[11]${NC} Fix SSL/Cert       ${CYAN}[15]${NC} Speedtest    ${CYAN}[19]${NC} Uninstall      ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${CYAN}[12]${NC} Optimize VPS       ${CYAN}[16]${NC} Update Script                  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}                                                                  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}  ${YELLOW}─── LAINNYA ─────────────────────────────────────────────────${NC}  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${CYAN}[0]${NC} Exit                ${CYAN}[99]${NC} Advanced Menu                  ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}   ${DIM}[help]${NC} Bantuan           ${DIM}Ketik 'menu' untuk buka menu${NC}            ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}                                                                  ${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
menu_advanced() {
    while true; do
        ...
        echo -e "${CYAN}║${NC}  ${CYAN}[3]${NC}  Auto Backup Config    ${CYAN}[9]${NC}  User IP Limits              ${CYAN}║${NC}"
        echo -e "${CYAN}║${NC}  ${CYAN}[4]${NC}  SSH Brute Protection  ${CYAN}[10]${NC} Custom Payload              ${CYAN}║${NC}"
...
_show_help() {
    ...
    echo -e "  ${CYAN}[99]${NC}    → Advanced settings"
